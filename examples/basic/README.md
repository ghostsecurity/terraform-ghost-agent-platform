...
Intended as a starting point — copy this directory and adapt for your environment. For production, pin the module `source` to a released tag (`source = "github.com/ghostsecurity/terraform-ghost-agent-platform?ref=<latest release tag, e.g. v0.2.0>"`) and swap the example's VPC/subnet for the network the rest of your account uses.
